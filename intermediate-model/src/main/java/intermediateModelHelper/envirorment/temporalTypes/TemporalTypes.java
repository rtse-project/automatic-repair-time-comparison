...
public class TemporalTypes {
    ...
    public int size(){
        filter();
        return rt_d.size() + rt_t.size() + et.size();
    }


    private void filter(){
        List<TimeMethod> rt_t_filter = new ArrayList<>();
        List<TimeMethod> rt_d_filter = new ArrayList<>();
        List<TimeParameterMethod> et_filter = new ArrayList<>();
        for(TimeMethod t : rt_d){
            if(rt_d_filter.contains(t)) continue;
            rt_d_filter.add(t);
        }
        for(TimeMethod t : rt_t){
            if(rt_t_filter.contains(t)) continue;
            rt_t_filter.add(t);
        }
        for(TimeParameterMethod t : et){
            if(et_filter.contains(t)) continue;
            et_filter.add(t);
        }
        rt_d = rt_d_filter;
        rt_t = rt_t_filter;
